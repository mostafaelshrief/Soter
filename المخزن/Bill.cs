...
namespace المخزن
{
    public partial class Bill : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            TotalRealPrice = Convert.ToString(TotalPrice);
            totalOut=Convert.ToDouble(itemOut.Text);
            totalOut+= totalOut;
            TotalItemOut=Convert.ToString(totalOut);
            Con.Open();
            string query = "update ItemTbl set خروج='" + TotalItemOut + "',الكميه='" + TotalRealAmount + "',totalOut='" + TotalRealPrice + "' where الاسم=N'" + ItemName.Text + "';";
            SqlCommand cmd = new SqlCommand(query, Con);
            cmd.ExecuteNonQuery();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                Con.Open();
                String query = "delete from HistoryTbl where الاسم=N'" + ItemName.Text + "';";
                SqlCommand cmd = new SqlCommand(query, Con);
                cmd.ExecuteNonQuery();
                MessageBox.Show("تم الحذف");
                Con.Close();
        ...
        {  
            ...
            {
              tot += Convert.ToInt32(dataGridView2.Rows[i].Cells[5].Value);
            }
            CountTotal.Text= tot.ToString();
        ...
        private void AddBtn_Click(object sender, EventArgs e)
        {
            ...
            Con.Open();
            String query = "insert into HistoryTbl values(N'" + BillSelectedlist.SelectedItem.ToString() + "','" + ItemName.Text + "','"+itemPrice.Text+"','"+itemOut.Text+"','"+ dateTimePicker1.Value.Date+ "','" + CountTotal.Text +"')";
            SqlCommand cmd = new SqlCommand("INSERT INTO HistoryTbl(الفاتورة,الاسم,السعر,خروج,الوقت,الاجمالى) VALUES" + "(@الفاتورة,@الاسم,@السعر,@خروج,@الوقت,@الاجمالى)", Con);
